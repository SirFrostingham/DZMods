modded class ZombieBase
{
	...
	void AfterDeath()
	{
		...
		if (lootDropChance > 0)
		{
			...
			if (RandomLootType == 0)
			{
				ItemAI = EntityAI.Cast( GetGame().CreateObject( m_PossibleLootDrops.GetRandomElement(), this.GetPosition(), false, true ) );
				ItemBS = ItemBase.Cast(ItemAI);
			}
			else
			{
				string ClassName = m_PossibleWeaponDrops.GetRandomElement();
				ItemAI = EntityAI.Cast( GetGame().CreateObject( ClassName, this.GetPosition(), false, true ) );
				ItemBS = ItemBase.Cast(ItemAI);
